...
const libdirac = Sys.iswindows() ? 
    joinpath(@__DIR__, "..", "lib", "windows", "bin", "libapproxLCD.dll") :
    Sys.isapple() ?
        joinpath(@__DIR__, "..", "lib", "mac", "bin", "libapproxLCD.dylib") :
        joinpath(@__DIR__, "..", "lib", "linux", "bin", "libapproxLCD.so")
...
function _init_instance()
    _dirac_dirac_short_double[] = LCDApproximationClassWrapper(
        ccall((:create_dirac_to_dirac_approx_short_double, libdirac), Ptr{Cvoid}, ())
    )
    _dirac_dirac_dynamic_weight_double[] = LCDApproximationClassWrapper(
        ccall((:create_dirac_to_dirac_approx_short_function_double, libdirac), Ptr{Cvoid}, ())
    )
    _gm_dirac_short_double[] = LCDApproximationClassWrapper(
        ccall((:create_gm_to_dirac_short_double, libdirac), Ptr{Cvoid}, ())
    )
...
function _destroy_instances()
    if _dirac_dirac_short_double[] !== nothing
        ccall((:delete_dirac_to_dirac_approx_short_double, libdirac), Cvoid, (Ptr{Cvoid},), _dirac_dirac_short_double[].ptr)
        _dirac_dirac_short_double[] = nothing
    end
    if _dirac_dirac_dynamic_weight_double[] !== nothing
        ccall((:delete_dirac_to_dirac_approx_short_function_double, libdirac), Cvoid, (Ptr{Cvoid},), _dirac_dirac_dynamic_weight_double[].ptr)
        _dirac_dirac_dynamic_weight_double[] = nothing
    end
    if _gm_dirac_short_double[] !== nothing
        ccall((:delete_gm_to_dirac_short_double, libdirac), Cvoid, (Ptr{Cvoid},), _gm_dirac_short_double[].ptr)
...
function dirac_short_double_approximate(
    ...
    success = ccall((:dirac_to_dirac_approx_short_double_approximate, libdirac), Bool,
...
function dirac_dynamic_weight_double_approximate(
    ...
    success = ccall((:dirac_to_dirac_approx_short_function_double_approximate, libdirac), Bool,
...
function gm_short_double_approximate(
    ...
    success = ccall((:gm_to_dirac_short_double_approximate, libdirac), Bool,
